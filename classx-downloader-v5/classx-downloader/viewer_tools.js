...
(() => {
  ...
  function layoutGrid(pagesPerSheet) {
    ...
    if (pagesPerSheet === 6) return { cols: 2, rows: 3 };
  ...
  async function convertPdfWithToggles(payload = {}) {
    ...
    // 300 DPI style output for sharper text in packed A4 sheets.
    const sheetPixelScale = 300 / 72;
    ...
    for (let outPage = 1; outPage <= outputPageCount; outPage += 1) {
      ...
      for (let pageNumber = pageStart; pageNumber <= pageEnd; pageNumber += 1) {
        ...
        const targetLongSide = Math.max(slotLongSide * 1.5, 1400);
        const renderScale = Math.max(0.9, Math.min(4.2, targetLongSide / Math.max(1, maxSide)));
      ...
      const jpegQuality = pagesPerSheet === 1 ? 0.97 : 0.95;
